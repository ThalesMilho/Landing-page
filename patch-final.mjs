...
// ── STEP 3: NavDropdown component (insert before Header) ──────────────────────
      ...
      {open && (
        <div style={{
          ...
          {item.children.map((child, idx) => child.external ? (
            ...
            <button key={idx} onClick={() => { go(child.p); setOpen(false); }}
              style={{
...
// ── STEP 4: Replace old nav renderer with exact string match ──────────────────
// This is the EXACT text from lines 270-300 of the restored file
const OLD_RENDERER = `            {navItems.map(({ label, p, external }) => {
              const active = page === p;
              if (external) return (
...
if (!f.includes(OLD_RENDERER)) {
  console.error('ERROR: Old renderer not found — exact whitespace mismatch.');
  console.log('Searching for anchor...');
  console.log('Has "navItems.map(({ label":', f.includes('navItems.map(({ label'));
  console.log('Has "navItems.map((item":', f.includes('navItems.map((item'));
  process.exit(1);
}

f = f.replace(OLD_RENDERER, NEW_RENDERER);
